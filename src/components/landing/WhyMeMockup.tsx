...
function SwipeableRow({
  ...
  // Hint: peek left every 10s on the first card
  useEffect(() => {
    if (!showHint) return;
    const playHint = () => {
  ...
  const handleDragEnd = (_: unknown, info: PanInfo) => {
    ...
    // Swipe LEFT: negative offset / negative velocity
  ...
  return (
    ...
    >
      {/* Green check background — revealed on RIGHT side as card slides left */}
      <m.div
      ...
      {/* Draggable card — swipe left to dismiss */}
      <m.div
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="text-[13px] font-semibold text-[#2D2926] dark:text-[#FAF8F5] truncate">
              {t(`tasks.${task.key}.title`)}
            </div>
            <div className="text-[11px] text-stone-400 dark:text-stone-500 truncate">
              {t(`tasks.${task.key}.time`)}
            </div>
          </div>
        </div>
...
export default function WhyMeMockup() {
  ...
  const visibleTasks = visibleKeys
    .map((key) => ALL_TASKS.find((td) => td.key === key)!)
    .filter(Boolean);
  ...
  return (
    <m.div
      ...
      whileHover={isDesktop ? { rotate: 0, scale: 1.02 } : undefined}
      transition={{ type: "spring", stiffness: 100, damping: 20 }}
      className="hidden md:block w-full max-w-[520px] ml-auto relative"
    >
      {/* Browser Mockup Container */}
      <div className="bg-[#FAF8F5] dark:bg-[#1A1816] rounded-3xl border border-stone-200 dark:border-white/10 shadow-[0_30px_80px_-15px_rgba(179,75,68,0.12)] dark:shadow-[0_30px_80px_-15px_rgba(179,75,68,0.25)] overflow-hidden">
        {/* Browser Header */}
        <div className="h-10 border-b border-stone-200 dark:border-white/10 bg-white dark:bg-[#242220] flex items-center px-5 justify-between">
