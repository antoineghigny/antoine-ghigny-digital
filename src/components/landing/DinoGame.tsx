...
function drawDino(ctx: CanvasRenderingContext2D, r: GameState) {
  ...
  if (r.jumping) {
    // Rotation mapping: rotate significantly up when jumping, down when falling
    ctx.rotate(r.dVel * 0.035);
    // Squash & Stretch: visibly stretch vertically when moving fast
    const stretch = 1 + Math.abs(r.dVel) * 0.025;
    ctx.scale(1 / stretch, stretch);
  } else if (r.active && !r.over) {
    // Huge dynamic bobbing when running (cartoonish)
    const bounce = Math.abs(Math.sin(r.frame * 0.35)) * 4;
    ctx.translate(0, -bounce);
    ctx.rotate(Math.sin(r.frame * 0.2) * 0.05);
  }
  ...
  if (duck) {
    ...
    if (r.jumping) {
      headY += r.dVel < 0 ? -3.5 : 4;   // Huge head drag / anticipate
      armY += r.dVel < 0 ? 6 : -6;      // Massive arm windmilling anticipation
      armR = r.dVel * -0.15;            // Flailing arms!
    } else if (r.active && !r.over) {
      headY += Math.sin(r.frame * 0.45) * 3;
      armY += Math.cos(r.frame * 0.45) * 3;
      armR = Math.sin(r.frame * 0.45) * 0.4;
    }
